...
\newcommand{\nodeLabel}[2]{\mbox{$#1$-$#2$}}
\newcommand{\modelgraphlabel}[4]{$#1$ \\ $r=#2$ \\ $\upsilon=#3$ \\ $\lambda=#4$}
\newcommand{\scale}{8}
...
($QPT$).  We utilized the OJAlgo package (\cite{OJAL:2013}) to calculate the
...
\noindent\begin{minipage}[c]{\textwidth}
    ...
    \hline
	    States  & Trees  & $TGT_{2}$ & $QPT_{2}$ & $QST_{2}$ & $TGT_{1}$    & $QPT_{1}$    \\ \hline
	    81     & 978    & 0.12 	& 0.22  & 0.08  & 1.30 	    & 1.60    \\ \hline
	    288    & 4507   & 0.26	& 0.36  & 0.16  & 19.00  & 19.62   \\ \hline
	    640    & 27746  & 1.56	& 1.67  & 0.61  & 137.29  & 143.27  \\ \hline
	    125    & 321372 & 6.01	& 6.11  & 0.09  & 114.25  & 114.58  \\ \hline
	    1944   & 6328   & 1.35	& 1.51  & 8.27  & 6124.01  & 6358.85 \\ \hline
	\end{tabular}
	\captionof{table}{Number of trees, $TGT$, $QPT$ and $QST$ across our models and algorithms}
	\label{fig:times}
\end{minipage}
\\
\\*\indent
...
recorded the times $QPT_1$ and $QPT_2$.  As shown in Table~\ref{fig:times},
...
generating all trees, in other words, we do not approximate MTTF and SSU.
...
for the thresholds and plot the fraction of $TGT$ versus percent error in $MTTF$.
...
Figure~\ref{fig:graph3} plots the ($TGT$, $MTTF$) and ($TGT$, $SSU$) pairs for
each of the stopping criteria considered using different values for each
threshold. Both $\tau_n$ and $\tau_h$ are fixed to be Real numbers, on the
other hand, $\tau_r$ can be sliced into finer granularity. The true value of
the $MTTF$, obtained when all trees are generated, is 44.124, and this defines
the vertical asymptote to which the approximate $MTTF$s for each stopping
...
using $\tau_r$ define our efficient frontier.

\subsection{Choosing an appropriate $\tau_r$}
We will use the model in Figure~\ref{fig:model} to explain the algorithm in
finding samples of $\tau_r$. Choosing a large rate threshold allows many
smaller trees to fill into the $Q$-matrix.  These smaller trees will have
large rates, since these tree rates include fewer products of $\phi$. However,
the diagonal of the $Q$-matrix is the sum of the whole row, which means many
larger trees can effect the diagonal entry.

In order to guage which trees will effect the dependability measures, we
utilize Djikstra's shortest path algorithm on a modified version of the model.
We would like to traverse the graph such that we obtain a valid failure tree,
and the rate of this tree will represent a threshold.  Unfortunately,
Djikstra's algorithm only works for summation of the edges on a graph, while
our tree rate is a product of the edges. To account for this limitation, we
changed all the edges on our model: $\mid\ln(E)\mid$.
\\
\begin{minipage}[c]{\textwidth}
	\centering
	\begin{tikzpicture}[scale = 1, auto = left, every node/.style = {circle, draw = black, fill = gray!25}, bend angle = 90]
		...
		\node (C) at (\scale, 0) [align = center] {\modelgraphlabel{C}{4}{1}{0.01}};
	  	...
	  	\foreach \from/\phiEdge/\to in {A/1.61/B, A/0.92/C, B/1.20/A, B/1.61/C, C/1.20/A, C/0.92/B}
	    	\path (\from) edge [-triangle 60, bend left=15] node[fill=none, draw=none] {\phiEdge} (\to);
	\end{tikzpicture} 
	\captionof{figure}{Graph representation of the 125-state model}
	\label{fig:model}
\end{minipage}
\\\\*
$Rates = i,j \in \Omega : j \in \Gamma_i (\lambda_i \prod {D(j)_E})$\\

\textbf{TODO} This equation is important, describe/prove equation or show example of usage?

\deleted{Starting with node A, we traverse to node B, and finally back to node A. Along
this path, we multiply all the edges, or $\phi$, and the lambda of the
starting node: $0.2 * 0.3 * 0.02 = 12e-4 = \tau_r$.}

\deleted{Starting with node B, we traverse to node A, and finally back to node B. Along
this path, we multiply all the edges, or $\phi$, and the lambda of the
starting node: $0.2 * 0.3 * 0.02 = 12e-4 = \tau_r$.}

\deleted{Starting with node C, we traverse to node A, then to B and finally back to
node C. Along this path, we multiply all the edges, or $\phi$, and the lambda
of the starting node: $0.3 * 0.3 * 0.02 = 12e-4 = \tau_r$.}

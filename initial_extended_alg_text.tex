...
% Sashank - a little more concise explanation
  \STATE \varName{BFHist} = $ [(\,), (\,), \ldots, (\,)] $; \COMMENT {Array of linked lists that keeps a history of parent component types in breadth-first order; BFHist is indexed by component typer}
  \STATE add \varName{rootC} to \varName{level};
  \STATE \varName{nFailed}[\varName{rootC}] = 1;
% Sashank - also a bit more concise
  \STATE add @ to \varName{BFHist}[\varName{rootC}]; \COMMENT {We can assume one component of type \varName{rootC} has failed}
...
% Sashank - enforce non-empty Gamma sets, this also means we don't need a special if-statement for empty nextLevelPossibilities
\\*\COMMENT{Builds set of all possible nodes in next level as Cartesian product of power sets of non-empty $\Gamma$ sets}
    ...
    \FOR{\varName{parentC} $\in$ \varName{level}}
      \FOR{\varName{childC} $\in$ $\Gamma_{\varName{parentC}}$}
          \IF{\varName{childC} $\in$ \varName{oneNextLevelPossibility}}  
            ...
            \STATE add @ to \varName{BFHist}[\varName{childC}]; \COMMENT {One component of type childC has failed}
            \STATE $\rho = \rho * \phi_{parentC,\; childC};$ 
% Sashank - the old phrasing was a bit confusing
            \\*\COMMENT {Update rate with appropriate phi}
          \ELSE
           \STATE add \varName{parentC} to \varName{BFHist}[\varName{childC}]; \COMMENT {One component of type childC has not failed, but was present in $\Gamma_{\varName{parentC}}$}
         \ENDIF
      \ENDFOR
    \ENDFOR
    %\STATE % empty line %
...
\FOR{$x' \in S'$}
  ...
  \FOR{\varName{comp} $\in \Omega$}
    ...
    \FOR{$\varName{parentC} \in \varName{BFHist}[\varName{comp}]$}
      ...
      \ELSE
        \STATE break;
% Sashank - modified this text a bit
        \COMMENT{compsAvailable must be greater than 0 or else there cannot be any more failed nodes of Type(comp)}
      \ENDIF
    \ENDFOR
  \ENDFOR
  %\STATE % empty line %
...
% Sashank - Moved q(x,y) equation up here, since it wouldn't make sense to set rootFailureRate without updating qMatrix

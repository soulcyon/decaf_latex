  ...
  \FOR{\varName{oneNextLevelPossibility} $\in$ \varName{nextLevelPossibilities}}
    ...
    \FOR{\varName{parentC} $\in$ \varName{level}}
      \FOR{\varName{childC} $\in$ $\Gamma_{\varName{parentC}}$}
          \IF{\varName{childC} $\in$ \varName{oneNextLevelPossibility} and \varName{childC} $\in$ }  
            ...
            \STATE add @ to \varName{BFHist}[\varName{childC}]; \COMMENT {One component of type \varName{childC} has failed}
...
\FOR{$x' \in S'$}
  ...
  \FOR{$i \in \Omega$}
    ...
    \FOR{$\varName{parentC}\,$ in $\varName{BFHist}[\varName{i}]$}
      ...
      \ELSE
        \STATE break;
        \COMMENT{$c_{i}$ must be greater than 0 or else there cannot be any more failed nodes of type $i$}
      \ENDIF
    \ENDFOR
  \ENDFOR

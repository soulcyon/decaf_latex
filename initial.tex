...
in depth-first order. The depth-first traversal of $T$ is \nodeLabel{A}{1}, \nodeLabel{B}{2}, \nodeLabel{A}{4}, \nodeLabel{C}{5}, \nodeLabel{A}{6}, \nodeLabel{B}{7}, \nodeLabel{A}{8}, \nodeLabel{B}{10}, \nodeLabel{C}{11}, \nodeLabel{C}{9}, \nodeLabel{C}{3}. Initially, the number of components up of each type are $u_A = 2$, $u_B = 2$ and $u_C = 1$ \changed{as before}. In this traversal a component of type $C$ fails at ID 5, which makes $u_C = 0$.
...
\FOR{\varName{rootC.type} $\in \Omega$}
  ...
  \STATE $\rho = 1$; \COMMENT {\changed{initialize product of component-affected probabilities}}
...
\changed{\subsection{Computing a tree's rate revisited}}

\changed{We now show how to compute $\eta$ using \varName{BFHist}. Using AddTreeLevel \citeBlock{6}{15} the following \varName{BFHist} is built.}

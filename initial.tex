...
\changed{Given the bottom-most tree level,} \mbox{AddTreeLevel} determines all
the possibilities for the next level that can be added to the current tree by
taking the Cartesian product of the power sets of the nonempty $\Gamma_{i}$ for
each of the failed components at the current bottom level. For each possible
next level, we recursively call \mbox{AddTreeLevel}. We use $\mathcal{P}$ to
denote the power set-like operation on an ordered set. In our realization of the
power-set operation, components in each subset maintain their relative ordering
from the original set. \changed{Note that by only maintaining one bottom-most level
at a time, as opposed to passing the entire tree per recursive call, we save some memory.}
...
\noindent\begin{minipage}[c]{\textwidth}
	...
	\end{tabular}
	\captionof{times}{Number of trees, $TGT$, $QPT$ and $QST$ across our models and algorithms}
	\label{fig:times}
\end{minipage}
\\
\\*\indent
...
recorded the times for $QPT$.  As shown in \ref{fig:times}, there is a
polynomial increase in ($TGT$ - $QPT$), which occurs due to the increasing
number of states. The main bottleneck in the old algorithm is the tree
generation, which has been improved exponentially.
\\
\noindent\begin{minipage}[c]{\textwidth}
	\centering

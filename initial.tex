...
\\*\vspace{1em}
In an effort to improving the running time of our algorithm, we introduce certain thresholds to stop
generating trees based on simple test conditions.  To properly stop the recursion of the trees, we need to
insert the threshold logic in Algorithm 2 before line 19.  We will denote the new threshold pseudo-code with
lines [19.x].   The breaking conditions will cause the program to continue to the next enumeration of
...
tree reaches $\tau_h$.  Rather than implementing a seperate variable for detecting the height of the current level, we will utilize \varName{level}.\\

\begin{algorithmic}
\IF{$\varName{level}.length > \tau_h$}
 	\STATE goto line 2;
\ENDIF
\end{algorithmic}

\vspace{1em}
The second threshold we consider is the \textit{node threshold}, where trees will contain a maximum of $\tau_n$ components.  To properly keep track of the number of components, we will use the sum of the components failed in \varName{nFailed}.\\
...
\vspace{1em}
The final threshold we analyzed is the \textit{rate threshold}, where we stop generating trees after the $\rho$ drops below $\tau_r$.\\
...
\vspace{1em}
The range of values for any $\tau$ is from 0 to $\infty$, however it is important to note that $\tau_h$ and $\tau_n$ depend on variables that increase as more trees are generated.  $\rho$ decreases as more components are attached, thus $\tau_r$ will need to be zero to generate all trees, while $\tau_h$ and $\tau_n$ need to be $\infty$.  In order to define a more appropriate range for our thresholds, we need to define the thresholds based on each model.   For initial loose bounds, we can assume the following limits: $\tau_n < \displaystyle\sum^i r_i$, $\tau_h < COUNT(LP)$ and $\tau_r < MAX(r_i \lambda_i)$.

To define the longest path ($LP$), we need to first describe our models as a graph data structure.  The graph's nodes are the component types, where each node's value is the $\lambda$ of that component type.  We define the directed edges to start at the parent component type and end at the cascading component type.  The value of each edge is $\phi_{i,j}$, where the edge starts at the ith node and ends at the jth node.  Since we know the longest path problem is not an NP-complete problem, we decided that the $\tau_h$ is not a reasonable threshold to pursue.
